class SoloAT0791 < Formula
  desc "An opinionated CLI tool to deploy and manage standalone test networks."
  homepage "https://github.com/hiero-ledger/solo"

  url "https://registry.npmjs.org/@hiero-ledger/solo/-/solo-0.79.1.tgz"
  sha256 "695c8cd027c424d71552949464824bc47d58faf8e201870732a7062204e4885c"
  version "0.79.1"
  ...
  def install
    ...
    # Step 4b: Warm default SoloAT0791 images to reduce first-run setup time.
  ...
  def pull_default_cache_images
    ...
    ohai "Pre-pulling default SoloAT0791 cache images..."
      ...
      loop do
        ...
        when :wait_readable
          ...
          if (Time.now - last_progress_log_at) >= progress_interval_seconds
            ohai "Still pre-pulling SoloAT0791 cache images... #{elapsed_seconds}s elapsed."
    ...
    opoo <<~EOS
      ATTENTION: Could not pre-pull SoloAT0791 cache images during install (#{e.message}).
  ...
  def collect_diagnostic_logs(solo_bin)
    ...
    diagnostics_commands.each do |command|
      begin
        ohai "Collecting SoloAT0791 diagnostic logs with `#{command.join(' ')}`..."
    ...
    opoo "ATTENTION: Unable to collect SoloAT0791 diagnostic logs automatically." unless diagnostics_collected
  end
