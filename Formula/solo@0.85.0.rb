class SoloAT0850 < Formula
  desc "An opinionated CLI tool to deploy and manage standalone test networks."
  homepage "https://github.com/hiero-ledger/solo"

  url "https://registry.npmjs.org/@hiero-ledger/solo/-/solo-0.85.0.tgz"
  sha256 "e726b42ec1d19d562c87725488a9f1072a75e7e2cd517d5f5299dee8cf9f722d"
  version "0.85.0"
  ...
  def install
    ...
    # Step 4b: Warm default SoloAT0850 images to reduce first-run setup time.
  ...
  def caveats
    <<~EOS
      WARNING: Homebrew support for SoloAT0850 is being removed.
  ...
  def pull_default_cache_images
    ...
    ohai "Pre-pulling default SoloAT0850 cache images..."
      ...
      loop do
        ...
        when :wait_readable
          ...
          if (Time.now - last_progress_log_at) >= progress_interval_seconds
            ohai "Still pre-pulling SoloAT0850 cache images... #{elapsed_seconds}s elapsed."
    ...
    opoo <<~EOS
      ATTENTION: Could not pre-pull SoloAT0850 cache images during install (#{e.message}).
  ...
  def collect_diagnostic_logs(solo_bin)
    ...
    diagnostics_commands.each do |command|
      begin
        ohai "Collecting SoloAT0850 diagnostic logs with `#{command.join(' ')}`..."
    ...
    opoo "ATTENTION: Unable to collect SoloAT0850 diagnostic logs automatically." unless diagnostics_collected
  end
