class SoloAT0800 < Formula
  ...
  def install
    ...
    # Step 4b: Warm default SoloAT0800 images to reduce first-run setup time.
  ...
  def pull_default_cache_images
    ...
    ohai "Pre-pulling default SoloAT0800 cache images..."
      ...
      loop do
        ...
        when :wait_readable
          ...
          if (Time.now - last_progress_log_at) >= progress_interval_seconds
            ohai "Still pre-pulling SoloAT0800 cache images... #{elapsed_seconds}s elapsed."
    ...
    opoo <<~EOS
      ATTENTION: Could not pre-pull SoloAT0800 cache images during install (#{e.message}).
  ...
  def collect_diagnostic_logs(solo_bin)
    ...
    diagnostics_commands.each do |command|
      begin
        ohai "Collecting SoloAT0800 diagnostic logs with `#{command.join(' ')}`..."
    ...
    opoo "ATTENTION: Unable to collect SoloAT0800 diagnostic logs automatically." unless diagnostics_collected
  end
