class SoloAT0821 < Formula
  desc "An opinionated CLI tool to deploy and manage standalone test networks."
  homepage "https://github.com/hiero-ledger/solo"

  url "https://registry.npmjs.org/@hiero-ledger/solo/-/solo-0.82.1.tgz"
  sha256 "4a4394a8e25d2cd92582e0f76dabe6a2cf8e3bf66425ed983a0b2f7e0c741083"
  version "0.82.1"
  ...
  def install
    ...
    # Step 4b: Warm default SoloAT0821 images to reduce first-run setup time.
  ...
  def pull_default_cache_images
    ...
    ohai "Pre-pulling default SoloAT0821 cache images..."
      ...
      loop do
        ...
        when :wait_readable
          ...
          if (Time.now - last_progress_log_at) >= progress_interval_seconds
            ohai "Still pre-pulling SoloAT0821 cache images... #{elapsed_seconds}s elapsed."
    ...
    opoo <<~EOS
      ATTENTION: Could not pre-pull SoloAT0821 cache images during install (#{e.message}).
  ...
  def collect_diagnostic_logs(solo_bin)
    ...
    diagnostics_commands.each do |command|
      begin
        ohai "Collecting SoloAT0821 diagnostic logs with `#{command.join(' ')}`..."
    ...
    opoo "ATTENTION: Unable to collect SoloAT0821 diagnostic logs automatically." unless diagnostics_collected
  end
