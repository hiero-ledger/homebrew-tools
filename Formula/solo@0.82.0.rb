class SoloAT0820 < Formula
  desc "An opinionated CLI tool to deploy and manage standalone test networks."
  homepage "https://github.com/hiero-ledger/solo"

  url "https://registry.npmjs.org/@hiero-ledger/solo/-/solo-0.82.0.tgz"
  sha256 "4e4322a20fc22e751f21edc106ea37342956ade2672f12a57ed4f54a13bc8fc0"
  version "0.82.0"
  ...
  def install
    ...
    # Step 4b: Warm default SoloAT0820 images to reduce first-run setup time.
  ...
  def pull_default_cache_images
    ...
    ohai "Pre-pulling default SoloAT0820 cache images..."
      ...
      loop do
        ...
        when :wait_readable
          ...
          if (Time.now - last_progress_log_at) >= progress_interval_seconds
            ohai "Still pre-pulling SoloAT0820 cache images... #{elapsed_seconds}s elapsed."
    ...
    opoo <<~EOS
      ATTENTION: Could not pre-pull SoloAT0820 cache images during install (#{e.message}).
  ...
  def collect_diagnostic_logs(solo_bin)
    ...
    diagnostics_commands.each do |command|
      begin
        ohai "Collecting SoloAT0820 diagnostic logs with `#{command.join(' ')}`..."
    ...
    opoo "ATTENTION: Unable to collect SoloAT0820 diagnostic logs automatically." unless diagnostics_collected
  end
